...
export default function Services() {
    return (
        <div
            className="relative flex size-full flex-col bg-transparent justify-between group/design-root overflow-x-hidden"
            style={{ fontFamily: 'Manrope, "Noto Sans", sans-serif' }}
        >
            {/* Services Section */}
            <div className="py-12 px-6 sm:px-8 md:px-12 lg:px-16 text-white">
                <h1 className="text-white text-3xl sm:text-4xl font-black text-center mb-8 sm:mb-10">
                    Services
                </h1>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 max-w-4xl mx-auto">
                    {[
                        { title: "Elevations", icon: "🏠" },
                        { title: "Architectural Plans", icon: "📐" },
                        { title: "3D Renders", icon: "🖼️" },
                        { title: "Consulting", icon: "🧠" },
                    ].map((service, idx) => (
                        <div
                            key={idx}
                            className="border border-[#f2730c] p-4 sm:p-6 rounded-xl flex items-center gap-3 sm:gap-4 hover:scale-[1.02] sm:hover:scale-[1.03] transition-transform duration-300"
                        >
                            <div className="text-2xl sm:text-3xl">{service.icon}</div>
                            <div className="text-lg sm:text-xl font-semibold">{service.title}</div>
                        </div>
                    ))}
                </div>
            </div>
